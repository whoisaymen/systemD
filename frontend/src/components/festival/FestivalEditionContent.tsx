...
const FestivalEditionContent: React.FC<FestivalEditionContentProps> = ({
	...
	const festivalDescription = getLocalizedValue(festival.description, language)
	const jurySection =
		juryMembers.length > 0 ? (
			<Accordion
				...
				onValueChange={(value) => setIsJuryOpen(value === 'jury')}
			>
				<AccordionItem
					value="jury"
				...
				>
					<AccordionTrigger
						data-active={isJuryOpen}
						className="festival-section-label relative -rotate-[2deg] hover:border-grayDark lg:text-5xl"
					>
						Jury
					</AccordionTrigger>
					<AccordionContent className="w-full">
						<ul className="my-4 grid w-full grid-cols-1 gap-4 lg:grid-cols-3 lg:px-16">
							{juryMembers.map((member: any, index: number) => {
								const biography = getLocalizedValue(member.biography, language)
								const memberKey = member._id ?? `${member.name}-${index}`

								return (
									<li
										key={memberKey}
										className="rounded-lg bg-grayDark p-3 text-dark"
									>
										<div className="relative">
											{member.image?.asset ? (
												<Img
													image={member.image}
													alt={member.name}
													className="aspect-[4/3] w-full rounded-lg border-2 border-primary object-cover lg:border-dark"
												/>
											) : (
												<div className="flex aspect-[4/3] w-full items-center justify-center rounded-lg border-2 border-dark bg-primary">
													<span className="text-5xl font-black italic text-dark">
														{member.name?.slice(0, 1)}
													</span>
												</div>
											)}
											<h3 className="relative z-10 mx-auto -mt-5 w-fit -rotate-2 rounded-md bg-primary px-2 text-center text-xl font-black italic leading-tight tracking-tight text-dark">
												{member.name}
											</h3>
										</div>

										{biography && (
											<p className="mt-4 text-base font-normal leading-[1.2] tracking-tight lg:text-lg">
												{biography}
											</p>
										)}
									</li>
	...
	return (
		<div className="no-scrollbar lg:shadowtest relative flex h-full w-full flex-col space-y-4 rounded-md px-4 py-24 pt-0 text-base font-medium leading-tight tracking-tighter text-primary sm:justify-start sm:space-y-1 sm:px-0 sm:pt-1 lg:mt-1 lg:h-[calc(100svh-10px)] lg:w-full lg:overflow-y-auto lg:rounded-lg lg:bg-dark lg:py-16">
			...
			<div className="mb-8 pt-8 lg:pt-0" id="film-selection">
				<div
					className="festival-section-layer sticky top-[0px] flex flex-col items-center justify-center bg-dark pt-2 lg:-top-[64px] lg:mx-1 lg:bg-dark lg:pt-4"
			...
			<Accordion
				...
				onValueChange={setPhotoExhibitionValue}
			>
				<AccordionItem
					value="item-3"
					className="festival-section-layer relative -mt-5 flex flex-col items-center justify-center pb-1 lg:-mt-2"
			...
			<div className="relative pb-0 pt-0" id="photo-gallery">
				<div
				...
				>
					<motion.div
						...
						data-active={isPhotoGalleryOpen}
						className={`festival-section-label relative flex w-fit cursor-pointer items-center justify-center gap-1 rounded-md border-[3px] px-2 pr-4 text-center text-4xl font-bold uppercase italic tracking-[-0.06em] shadow-sm transition-all hover:border-grayDark hover:bg-grayDark hover:text-dark lg:text-5xl ${
							isPhotoGalleryOpen
								? 'border-grayDark bg-grayDark text-dark'
								: 'border-primary bg-dark text-primary'
						}`}
