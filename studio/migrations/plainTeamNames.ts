...
async function run() {
	...
	const plans = documents
		...
		.map((document) => {
			if (
				...
				throw new Error(`Unexpected name content: ${document._id}`)
			return {
				document,
				name: richTextToPlainText(document.name).replace(/\s*\r?\n\s*/g, ' '),
			}
		})
	for (const { document, name } of plans) {
		console.log(`${document._id}: ${JSON.stringify(name)}`)
	}
	console.log(`${plans.length} names ${apply ? 'to convert' : 'would change'}.`)
	...
	const backupPath = `${backupDirectory}/team-names-${Date.now()}.json`
	fs.writeFileSync(
		backupPath,
		JSON.stringify(
	...
	for (const { document, name } of plans) {
		transaction = transaction.patch(document._id, (patch) =>
			patch.ifRevisionId(document._rev).set({ name }),
		)
	}
	...
	console.log('All names verified. Draft and published states preserved.')
}
